...
const deleteAnAdmin = asyncHandler(async (req, res) => {
	...
	await Order.updateOne({ _id: adminId }, { $set: { isAdmin: false, isSuperAdmin: false } });
...
const updateAnAdmin = asyncHandler(async (req, res) => {
	...
	await User.updateOne({ _id: adminId }, { $set: { username, password, isAdmin } });

	res.json({ success: true });
});

module.exports = { login, getUser, deleteAnAdmin, getAllAdmins, register, updateAnAdmin };
